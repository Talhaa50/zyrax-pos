...
export default function POSPage() {
  ...
  useEffect(() => { searchProducts(query).then(setProducts); }, [query]);
    ...
    <div className="grid min-h-[calc(100vh-3.75rem)] bg-surface-secondary dark:bg-black lg:grid-cols-[minmax(0,1fr)_24rem]">
      <div className="flex min-w-0 flex-col border-r border-black/[0.04] dark:border-white/[0.06]">
        <div className="border-b border-black/[0.04] bg-white/75 p-4 backdrop-blur-ios dark:border-white/[0.06] dark:bg-surface-dark/75">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex gap-2 text-xs font-semibold"><span className="rounded-full bg-emerald-500/10 px-3 py-1.5 text-emerald-700 dark:text-emerald-300">{inStock.length} in stock</span><span className="rounded-full bg-brand-500/10 px-3 py-1.5 text-brand-700 dark:text-brand-300">{cart.itemCount} cart items</span></div>
          </div>
          <input ref={searchRef} type="text" placeholder="Search products or scan barcode..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={handleBarcodeScan} className="w-full rounded-2xl border border-black/[0.06] bg-white px-5 py-3.5 text-base font-medium shadow-ios transition-all duration-200 ease-ios placeholder:text-gray-400 focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-500/25 dark:border-white/[0.08] dark:bg-white/[0.04] dark:placeholder:text-gray-500" />
        </div>
      ...
      <div className="flex min-h-[32rem] flex-col bg-white/90 backdrop-blur-ios dark:bg-surface-dark/90">
        <div className="border-b border-black/[0.04] px-5 py-4 text-[0px] [&>h3]:hidden dark:border-white/[0.06]">
          <div className="flex items-center justify-between gap-3 text-base"><div><p className="text-xs font-semibold uppercase text-gray-400">Current sale</p><h3>Cart</h3></div><span className="rounded-full bg-black/[0.04] px-3 py-1.5 text-xs font-semibold text-gray-600 dark:bg-white/[0.08] dark:text-gray-300">{cart.itemCount} items</span></div>
